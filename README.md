...
Guardian ships a stdlib-only HTTP server that receives `marketplace_purchase` events when a customer upgrades, downgrades, or cancels Guardian on the GitHub Marketplace. The binary lives at `app/cmd/webhook/` and is packaged as a distroless image via `Dockerfile.webhook` for Cloud Run (or any container runtime).
...
`purchased`, `cancelled`, `changed`, `pending_change`, and `pending_change_cancelled`. Any other action returns `422` so misconfigurations surface loudly. The body is capped at 1 MiB; signature failures return `400`; malformed JSON returns `400`.

### Build and run locally

```bash
docker build -f Dockerfile.webhook -t guardian-webhook .
